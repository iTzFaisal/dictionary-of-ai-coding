...
Add a dedicated workflow that installs dependencies, builds the static site, uploads the generated artifact, and deploys it using GitHub Pages Actions. It will run on pushes to `main` and support manual dispatch. The existing README freshness and webhook workflows remain separate.
...
4. Deploy from `main`, then verify the index, a direct term URL, internal links, mobile navigation, and search at the project Pages URL.
